...
const loadData = async()=>{
    try {
    const response = await api.get("/admin/view/all/students");
...
if(search.value.trim() === ""){
  ...
  return students.value.filter((student)=>{
   return student.name.toLowerCase().includes(search.value.toLowerCase()) || String(student.id).includes(search.value)
  });
});
    ...
    <div class="main">
        <div class="left">
            ...
            <div class="links">
                ...
                <router-link to="/admin/view/students"><button class="btn1">Students</button></router-link>
        ...
        <div class="right">
            <div class="right_left">
                <div class="right_head">
                    <p class="right-head">Students</p>
                    <input type="search" class="search" placeholder="Search here" v-model="search">
                </div>
                 <p class="no_data" v-if="(students.length) === 0 || searchStudent.length === 0">No registered student</p>
                <div class="right_main_body" tabindex="0" v-if="(students.length) !== 0 && searchStudent.length !==0">
                    <div class="titles">
                        <p>Id</p>
                        <p>Name</p>
                        <p>Email</p>
                    ...
                    <div v-for="student in searchStudent" class="values" @click="viewName(student.id)">
                        <p>{{ student.id }}</p>
                        <p>{{ student.name }}</p>
                        <p>{{ student.email }}</p>
                        <p>{{ student.status }}</p>
            ...
            <div class="right_right" v-if="check">
                ...
                <div class="links3">
                 <button class="det" :class="{noName:!name}" @click="viewDetail(id)">View Details</button>
                    <button class="det" :class="{noName:!name}" @click = "blackuser(id)">Blacklist</button>
